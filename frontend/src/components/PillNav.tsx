...
const PillNav = ({
  ...
  const isItemActive = (href: string) => {
    ...
    if (href.startsWith('#')) {
      return href === hash || (href === '#about' && !hash && location === '/');
    }
    if (href.startsWith('/')) {
      const cleanHref = href.split('#')[0];
      const cleanLoc = location.split('#')[0];
      return cleanLoc === cleanHref;
    }
  ...
  useEffect(() => {
    const layout = () => {
      circleRefs.current.forEach((circle, index) => {
        ...
        if (isItemActive(items[index].href)) {
          tl.progress(1);
        }
      });
    };
  ...
  }, [activeHash, location, activeHref, items, ease]);
  ...
  const handleLeave = (i: number) => {
    if (isItemActive(items[i].href)) return;
    const tl = tlRefs.current[i];
    if (!tl) return;
    activeTweenRefs.current[i]?.kill();
    activeTweenRefs.current[i] = tl.tweenTo(0, {
  ...
  return (
    <div className="pill-nav-container">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        ...
        {rightElement && <div className="pill-nav-right desktop-only" style={{ marginLeft: 'auto' }}>{rightElement}</div>}
